...
::
    ...
    blobstore://[<api_key>][:<secret>[:<security_token>]]@<hostname>[:<port>]/<name>?bucket=<bucket_name>[&prefix=<key_prefix>][&region=<region_name>][&<param>=<value>]...]
      ...
      <key_prefix> - Optional object key prefix.  When present, the backup's object key trees ('data/' and 'backups/') are placed under this prefix in the bucket instead of the bucket root.  Each path segment may contain only ASCII alphanumerics, '_', '-' and '.'; '/' separates segments.  Leading and trailing '/' are ignored, while empty, '.' and '..' path segments and the reserved first segments 'data' and 'backups' are rejected.
 ...
 *sdk_auth* (or *sa*) - Use the AWS SDK to do credentials and authentication. This supports all aws authentication types, including credential-less iam role-based authentication in aws. Experimental, and only works if FDB was compiled with BUILD_AWS_BACKUP=ON. When this parameter is set, all other credential parts of the backup url can be ignored.  By default the SDK's default provider chain supplies the credentials (environment variables, the profile file, process credentials, web identity, SSO, the ECS container endpoint, then the EC2 instance metadata service); the three parameters below refine this and each of them implies ``sdk_auth=1``.  The https proxy of the environment (``https_proxy``) is used for the STS call of ``role_arn``.

 *role_arn* - ARN of an IAM role to assume through STS ``AssumeRole`` (regional STS endpoint of the blobstore's ``region``).  The base identity, selected by *credentials_provider_type*, needs ``sts:AssumeRole`` on the role and the role's trust policy must allow it; grant the bucket permissions to the role.  The SDK renews the role session (15 minutes) by itself.  Cannot be combined with access keys in the URL.  Same meaning as the ``s3.role_arn`` setting of Doris.

 *external_id* - External ID sent with ``AssumeRole`` when the role's trust policy requires one.  Needs *role_arn*.  Same meaning as the ``s3.external_id`` setting of Doris.

 *credentials_provider_type* - Which SDK provider supplies the (base) credentials: ``DEFAULT`` (the default chain above), ``ENV`` (``AWS_ACCESS_KEY_ID`` / ``AWS_SECRET_ACCESS_KEY``), ``SYSTEM_PROPERTIES`` (the profile file), ``WEB_IDENTITY`` (``AWS_ROLE_ARN`` + ``AWS_WEB_IDENTITY_TOKEN_FILE``, e.g. EKS IAM roles for service accounts), ``CONTAINER`` (the ECS container credentials endpoint) or ``INSTANCE_PROFILE`` (the EC2 instance metadata service).  Same meaning as the ``s3.credentials_provider_type`` setting of Doris; its ``ANONYMOUS`` is not offered because a backup needs write access.

 *aliyun_auth* (or *aa*) - Set to 1 to resolve Alibaba Cloud OSS credentials with the Alibaba Cloud Credentials SDK instead of writing AK/SK into the URL.  Requests are still signed with the SigV4 algorithm and carry the STS security token, so ``region`` stays required (for example ``oss-cn-beijing``) and the host must use the virtual-hosted form ``<bucket>.oss-<region>.aliyuncs.com``.  By default the SDK's default chain supplies the credentials: ``ALIBABA_CLOUD_ACCESS_KEY_ID`` / ``ALIBABA_CLOUD_ACCESS_KEY_SECRET`` (/ ``ALIBABA_CLOUD_SECURITY_TOKEN``), the ACK RRSA variables (``ALIBABA_CLOUD_ROLE_ARN``, ``ALIBABA_CLOUD_OIDC_PROVIDER_ARN``, ``ALIBABA_CLOUD_OIDC_TOKEN_FILE``), the CLI and profile files, the ECS instance RAM role named by ``ALIBABA_CLOUD_ECS_METADATA``, then ``ALIBABA_CLOUD_CREDENTIALS_URI``; when that chain yields nothing, the ECS instance RAM role is tried with the role name discovered from the metadata service (unless ``ALIBABA_CLOUD_ECS_METADATA_DISABLED`` is set), so an instance with an attached role needs no configuration at all.  Cannot be combined with access keys in the URL, ``sdk_auth`` or ``gcp_auth``.  Requires a build with ``BUILD_ALIYUN_BACKUP=ON``.

 *aliyun_credential_provider_type* - Pins the source of the Alibaba Cloud credentials and implies ``aliyun_auth=1``: ``DEFAULT`` (the chain above), ``ENV`` (the ``ALIBABA_CLOUD_ACCESS_KEY_ID`` variables only), ``ECS_RAM_ROLE`` (only the RAM role attached to the ECS instance; ``ALIBABA_CLOUD_ECS_METADATA`` names the role when the instance has several) or ``OIDC_ROLE_ARN`` (only ACK RRSA).

 *gcp_auth* (or *ga*) - Set to 1 to authenticate to Google Cloud Storage with OAuth2 access tokens from the Google Cloud SDK instead of HMAC keys.  The URL must not contain credentials and needs no ``region``.  Tokens are refreshed automatically before they expire.  Requires a build with ``BUILD_GCP_BACKUP=ON``.  The SDK's environment variables apply, e.g. ``GCE_METADATA_ROOT`` overrides the metadata server address.  The two parameters below configure where the credential comes from; either of them implies ``gcp_auth=1``.

 *gcp_credential_provider_type* - ``DEFAULT`` (the default) uses the SDK's application default credentials lookup: the file named by ``GOOGLE_APPLICATION_CREDENTIALS`` (service account key, external account for Workload Identity Federation, or impersonated service account), then the gcloud ``application-default`` login, then the service account of the GCE VM or GKE pod (Workload Identity) through the metadata server.  ``COMPUTE_ENGINE`` asks only the metadata server, so credential files and developer logins on the host cannot change the identity; recommended for production on Compute Engine.  Same meaning as the ``gcs.credential_provider_type`` setting of Doris.

 *gcp_impersonation_service_account* - Email of a service account to impersonate, e.g. ``data-access@project.iam.gserviceaccount.com``.  The source credential (from ``gcp_credential_provider_type``) is exchanged for a short-lived token of that account through the IAM Service Account Credentials API, so Cloud Storage sees the impersonated account: grant it the bucket permissions, and grant the source identity ``roles/iam.serviceAccountTokenCreator`` on it.  Needs access to ``iamcredentials.googleapis.com``.  Same meaning as the ``gcs.impersonation_service_account`` setting of Doris.
